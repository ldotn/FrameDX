...
#define LogMsg(msg,cat) FrameDX::Log._record(msg,cat,__LINE__,MAKE_WIDE(__FUNCTION__),MAKE_WIDE(__FILE__))
...
#define LogCheck(cond,cat) {auto scode = (FrameDX::StatusCode)(cond); if(scode != FrameDX::StatusCode::Ok) FrameDX::Log._record(std::wstring(#cond L" failed with code ") + FrameDX::StatusCodeToString(scode),cat,__LINE__,MAKE_WIDE(__FUNCTION__),MAKE_WIDE(__FILE__)); }
...
#define LogAssertWithReturn(cond,cat,ret) if(!(cond)) { FrameDX::Log._record(#cond L" != true",cat,__LINE__,MAKE_WIDE(__FUNCTION__),MAKE_WIDE(__FILE__)); return ret; }
	...
	// This is a macro, so it can be used to return out of a function on failure
#define LogCheckWithReturn(cond,cat) {auto scode = ( FrameDX::StatusCode)(cond); if(scode != FrameDX::StatusCode::Ok) {FrameDX::Log._record(std::wstring(#cond L" failed with code ") + FrameDX::StatusCodeToString(scode) ,cat,__LINE__,MAKE_WIDE(__FUNCTION__),MAKE_WIDE(__FILE__)); return scode; }}
...
#define LogCheckAndContinue(cond,cat) [&](){auto scode = ( FrameDX::StatusCode)(cond); if(scode !=  FrameDX::StatusCode::Ok) { FrameDX::Log._record(std::wstring(#cond L" failed with code ") + FrameDX::StatusCodeToString(scode) ,cat,__LINE__,MAKE_WIDE(__FUNCTION__),MAKE_WIDE(__FILE__));} return scode; }()
